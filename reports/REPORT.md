...
python src/dqn_cartpole.py
```

**交付物：**
- `src/dqn_cartpole.py`：完整训练代码
- `outputs/models/dqn_cartpole_policy.pt`：训练好的策略网络权重
- `outputs/logs/training_log.csv`：逐回合日志（reward / epsilon / loss / success）
- 奖励曲线、成功率曲线截图
...
![Reward Curve](../outputs/figures/reward_curve.png)
...
训练结束后加载 `outputs/models/dqn_cartpole_policy.pt`，关闭探索，跑 3 个测试回合：
...
`outputs/logs/training_log.csv` 字段：`episode, reward, epsilon, loss_mean, success`，可用 Pandas 或 Excel 复现图表。
